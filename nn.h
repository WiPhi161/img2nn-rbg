...
//         float sum=0;
//         for (size_t j = 0; j < q; ++j) {
//             float d = expf(MAT_AT(NN_OUTPUT(nn), 0, j));
//             sum+=d;
//         }
//         int actual_label;
//         for (size_t j = 0; j < q; ++j) {
//             if (MAT_AT(y, 0, j) > 0.0f){
//                 actual_label=j;
//                 break;
//             }
//         }
//         float prob = expf(MAT_AT(NN_OUTPUT(nn), 0, actual_label))/sum;
//         log_likelihood+= logf(prob);
//     }
